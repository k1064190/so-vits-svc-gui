...
def inference(
    ...
    Args:
        model_path: model path for so-vits
        config_path: config path for so-vits
        cluster_model_path: model path for cluster model
        feature_retrieval: true if use feature retrieval
        raw_audio_path: path to raw audio
        spk: list of spk [spk1, spk2, ...]
        tran: transpose, 0 for no transpose, 1 for transpose up, -1 for transpose down(pitch)
        slice_db: slice the silent part of the audio if the db is lower than slice_db
        cluster_infer_ratio: ratio of cluster model inference
        auto_predict_f0: use auto predict f0 in sovits. Turn off when converting singing voice
        f0_predictor: [crepe, rmvpe, fcpe]
        noice_scale: inject the noise in the latent space to increase the diversity of the output
        pad_seconds: pad the chunk with silence for pad_seconds
        clip_seconds: clip the audio to clip_seconds(0 for no clip)
        lg_num: crossfade length in seconds
        lgr_num: crossfade retain ratio(real crossfade length, remaining length will be cut)
        enhancer_adaptive_key: adapt the enhancer to higher pitch(not implemented yet)
        cr_threshold: f0 filter threshold(only for crepe)
        k_step: diffusion step (not implemented yet)
        use_spk_mix: if true, use multiple spk on different timeline (not implemented yet)
        spk_mix_map: Map a spk to a timeline(linear) (not implemented yet)
        second_encoding: use second encoding before diffusion
        loudness_envelope_adjustment: adjust the loudness envelope of the output(0 for original, 1 for output)
        shallow_diffusion: use shallow diffusion (not implemented yet)
        only_diffusion: only use diffusion model (not implemented yet)
        diffusion_model_path: model path for diffusion model
        diffusion_config_path: config path for diffusion model
        enhance: use NSF_HIFIGAN enhancer (not implemented yet)
    ...
    # "logs/44k/diffusion/model_0.pt", "configs/diffusion.yaml"
    #
    # Load sovits model
